...
with tf.Session() as sess:
	...
	for i in xrange(len(training_trajectories)):
	    ...
	    r = -abs(1.0 * current_box_coord - training_trajectories[i][-1].index(1))
	    rewards = np.full(len(training_trajectories[i]) - 1, r * 1.0 / 10)
            prev_box = training_trajectories[i][0]
            ...
            if (i + 1) % 2000 == 0:
                print str(i + 1)
                saver.save(sess, '../QL_VII_ckpts/relu/' + str(k), global_step=i+1)
