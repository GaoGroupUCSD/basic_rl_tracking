...
# Unlike the previous method, epsilon-greedy policy selection has been used.
    ...
    while True:
        ...
        for t in trajectories:
            ...
            for i in xrange(1, len(trajectories[t])):
                ...
                epsilon = 1.0 / (5 + 0.01 * num_episodes)

                random_sample = np.random.uniform()

                majority_action = np.argmax(output)
                ...
                chosen_action = majority_action
                
                if random_sample <= epsilon:
                    while chosen_action == majority_action:
                        chosen_action = np.random.randint(0, 3)

                action_array = [0, 0, 0]
                action_array[chosen_action] = 1

                random_step_size = step_sizes[np.random.randint(0, len(step_sizes))]
                current = np.array([[prev[0][0] + (-action_array[0] + action_array[2])*random_step_size, prev[0][1]]])

		print output
		print action_array
		print

                action_stack.append(np.array([action_array]))
