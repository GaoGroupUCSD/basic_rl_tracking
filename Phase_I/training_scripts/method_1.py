# This method trains the network on 76 horizontal trajectories going left to right
...
out = cv2.VideoWriter('training_horizontal_2.avi',fourcc, 20.0, (240, 80))
            ...
            for i in xrange(1, len(trajectories[t])):
                ...
                if i == len(trajectories[t]) - 1:
                    ...
                    for j in xrange(len(y_stack)-1, -1, -1):
                        #print j
                        sess.run(y.assign(y_stack[j] * overall_loss * pow(gamma, count)))
                        a = action_stack[j] * overall_loss * pow(gamma, count)
                        #print a
                        sess.run([train_individual], feed_dict={action: a})
                        count += 1
                    
                    saver.save(sess, './rl_model_2.ckpt')
